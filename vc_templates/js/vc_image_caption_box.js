(function ($) {
        ...
        changeShortcodeParams:function (model) {
          ...
          $wrapper = this.$el.find('> .wpb_element_wrapper');
            if (_.isObject(params)) {
                $wrapper.html("<h4 class=\"wpb_element_title\"><span class=\"vc_element-icon listeo_icon\"></span> Image with caption</h4> Title: "+ params.title+"; <br /> Subtitle: "+params.subtitle).css('height','auto');
            }
        }
    });
    window.VcHeadlineView = vc.shortcode_view.extend({
        changeShortcodeParams:function (model) {
          ...
          $wrapper = this.$el.find('> .wpb_element_wrapper');
            if (_.isObject(params)) {
                $wrapper.html("<h4 class=\"wpb_element_title\"><span class=\"vc_element-icon listeo_icon\"></span> Headline: "+ params.content+"</h4>").css('height','auto');
