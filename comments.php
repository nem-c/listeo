...
<section class="comments">
	...
	if ( have_comments() ) : ?>
		<h4 class="headline margin-bottom-35">
			<?php
				printf( // WPCS: XSS OK.
					esc_html( _nx( 'Comments %1$s', ' Comments %1$s', get_comments_number(), 'comments title', 'listeo' ) ),
		...
		<nav id="comment-nav-above" class="navigation comment-navigation" role="navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'listeo' ); ?></h2>
			<div class="nav-links">
		...
		<ul class="comment-list">
			...
			if ( get_post_type( get_the_ID() ) == 'listing' ) {
			    wp_list_comments( array(
					'style'      	=> 'ul',
					'short_ping' 	=> true,
					'callback' 		=> 'listeo_review',
				) );
			} else {
				wp_list_comments( array(
					'style'      	=> 'ul',
					'short_ping' 	=> true,
					'callback' 		=> 'listeo_comment',
				) );
			}
